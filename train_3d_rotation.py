...
def main():
    ...
    train_dl = RotationLoader(mode='train', img_size=64)
    train_loader = DataLoader(dataset=train_dl, batch_size=4, num_workers=4, shuffle=False)
    optimizer = optim.Adam(model.parameters(), lr=0.0001, amsgrad=True, weight_decay=1e-4)  # default as 0.0001
    val_dl = RotationLoader(mode='val', img_size=64)
    val_loader = DataLoader(dataset=val_dl, batch_size=4, num_workers=4, shuffle=True)
    ...
    for epoch in range(50):
        ...
        for i, (img, label, gt) in enumerate(pbar):
            ...
            print("%.4f" % loss, gt[0], pred[0])
            loss.backward()
            optimizer.step()
            pbar.set_postfix({'loss': '%.4f' % loss})

        model_save_path = os.path.join("F:\\Data\\mnist_3d", "exp4_resnet18_3d_%d.pth" % epoch)
